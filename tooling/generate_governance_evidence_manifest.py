...
def _generate_evidence_manifest(
    ...
    manifest = {
        ...
        "automated_reports": {
            "branch_protection_verification": _verify_github_branch_protection(artifacts_dir),
            "required_status_checks_consistency": _verify_required_status_checks_consistency(artifacts_dir),
        },
    ...
    policy_status = all(
        doc.get("status") == "present"
        for doc in manifest["policy_documents"].values()
    )
    ...
    manifest["exit_criteria"]["verification_passed"] = automated_status and policy_status
...
def main():
    ...
    if manifest["exit_criteria"]["verification_passed"]:
        ...
        print("⚠ Some automated reports not collected; manual evidence capture still needed")
        return 1
