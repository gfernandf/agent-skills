...
def _format_prometheus(snapshot: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("# HELP agent_skills_uptime_seconds Runtime uptime in seconds")
    lines.append("# TYPE agent_skills_uptime_seconds gauge")
...
class ServerConfig:
    ...
    trusted_proxies: frozenset[str] = (
        frozenset()
    )  # IPs of trusted reverse proxies for X-Forwarded-For
...
class _RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        try:
            ...
            if parsed.path == "/v1/metrics":
                ...
                if metrics is None:
                    self._write_json(
                        200, {"uptime_seconds": 0, "counters": {}, "histograms": {}}
                    )
                else:
            ...
            if parsed.path == "/v1/metrics/prometheus":
                ...
                body = _format_prometheus(
                    metrics.snapshot()
                    if metrics
                    else {"uptime_seconds": 0, "counters": {}, "histograms": {}}
                )
        ...
        try:
            ...
            if parsed.path == "/v1/skills/discover":
                ...
                if not isinstance(intent, str) or not intent:
                    raise ValueError(
                        "discover requires non-empty string field 'intent'"
                    )
            ...
            if parsed.path.startswith(skill_prefix) and parsed.path.endswith("/attach"):
                ...
                if not isinstance(target_type, str) or not target_type:
                    raise ValueError(
                        "attach requires non-empty string field 'target_type'"
                    )
                if not isinstance(target_ref, str) or not target_ref:
                    raise ValueError(
                        "attach requires non-empty string field 'target_ref'"
                    )
            ...
            if parsed.path.startswith(runs_prefix) and parsed.path.endswith("/approve"):
                store = self.run_store
                if store is None:
                    self._write_json(
                        501,
                    ...
                    return
                run_id = parsed.path[len(runs_prefix) : -len("/approve")].rstrip("/")
                response = self._api().approve_run(
                    run_id,
            ...
            if parsed.path.startswith(runs_prefix) and parsed.path.endswith("/deny"):
                store = self.run_store
                if store is None:
                    self._write_json(
                        501,
                    ...
                    return
                run_id = parsed.path[len(runs_prefix) : -len("/deny")].rstrip("/")
                response = self._api().deny_run(
                    run_id,
                    approver=(body.get("approver") if isinstance(body, dict) else None),
                    notes=(body.get("notes") if isinstance(body, dict) else None),
                    run_store=store,
            ...
            if parsed.path.startswith(runs_prefix) and parsed.path.endswith("/fork"):
                store = self.run_store
                if store is None:
                    self._write_json(
                        501,
                    ...
                    return
                run_id = parsed.path[len(runs_prefix) : -len("/fork")].rstrip("/")
                checkpoint_id = (
                    body.get("checkpoint_id")
                    if isinstance(body, dict)
                    and isinstance(body.get("checkpoint_id"), str)
            ...
            if parsed.path.startswith(skill_prefix) and parsed.path.endswith(
                ...
                trace_id = self._extract_trace_id(body)
                include_trace = (
                    bool(body.get("include_trace", False))
                    if isinstance(body, dict)
                    else False
                )
                required_profile = None
            ...
            if parsed.path.startswith(capability_prefix) and parsed.path.endswith(
                "/execute"
            ):
            ...
            if not middleware.authorize(identity, method, path):
                ...
                self._write_json(
                    403,
                    {
                        "error": {
                            "code": "forbidden",
                            "message": f"Role '{identity.role}' insufficient for this endpoint.",
    ...
    def _enforce_rate_limit(self, path: str) -> bool:
        ...
        with self._rate_lock:
            ...
            stale_clients = [
                cid
                for cid, timestamps in self._rate_state.items()
                ...
                if wh_store.unregister(sub_id):
                    ...
                    self._write_json(
                        404,
                        {
                            "error": {
                                "code": "not_found",
                                "message": f"Webhook '{sub_id}' not found",
                                "type": "NotFound",
                            }
                        },
                    )
...
def run_server(
    ...
    *,
    ...
    if auth_mode == "disabled":
        logger.warning(
            "Auth disabled (AGENT_SKILLS_AUTH_MODE=disabled) — not recommended for production"
        )
    else:
    ...
    if trusted_raw:
        config = ServerConfig(
            ...
            trusted_proxies=frozenset(
                p.strip() for p in trusted_raw.split(",") if p.strip()
            ),
        )
        _RequestHandler.config = config
