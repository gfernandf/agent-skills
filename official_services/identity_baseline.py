...
def identify_assignee(task, candidates=None):
    ...
    if not isinstance(task, dict):
        return {
            "assignee": pool[0] if pool else {},
            "alternatives": [],
            "rationale": "No task provided; returning first candidate.",
        }
    ...
    for c in pool:
        c = (
            ...
            else _PRINCIPALS.get(str(c), {"id": str(c), "skills": [], "workload": 5})
        )
    ...
    return {
        ...
        "alternatives": [
            {"id": s["candidate"].get("id"), "match_score": s["score"]}
            for s in scored[1:3]
        ],
...
def justify_decision(decision, subject, policies=None):
    ...
    applicable = [p for p in policies_list if isinstance(p, dict)] or [
        {"id": "default", "description": "Default identity policy"}
    ]
...
def gate_permission(principal_id, permission, context=None):
    ...
    if str(permission) in effective:
        return {
            "allowed": True,
            "reason": f"Permission '{permission}' granted via role.",
        }
    return {
        "allowed": False,
        "reason": f"Principal '{principal_id}' does not hold '{permission}'.",
    }
...
def list_permissions(principal_id, resource_filter=None):
    ...
    if resource_filter:
        perms = [
            p
            for p in perms
            if str(p.get("resource", "")).startswith(str(resource_filter))
        ]
...
def list_roles(scope=None):
    ...
    if scope:
        roles = [
            r
            for r in roles
            if scope in r.get("tags", []) or scope in r.get("description", "").lower()
        ]
