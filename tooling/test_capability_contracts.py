...
def _check_output_shape(
    capability_id: str, declared_outputs: dict, result: Any
) -> list[str]:
...
def _check_graceful_error(capability_id: str, binding, cap_spec) -> list[str]:
    ...
    try:
        success, reason, result = batch.call_capability(
            capability_id, binding, invalid_input
        )
        # We expect either:
        ...
        if result is None and success:
            violations.append(
                "Returned success=True but result is None with invalid input."
            )
    except Exception as e:
        violations.append(
            f"Unhandled exception with invalid input: {type(e).__name__}: {e}"
        )
...
def run_contract_tests():
    ...
    for capability_id in sorted(all_capabilities.keys()):
        ...
        if test_input:
            success, reason, result = batch.call_capability(
                capability_id, binding, test_input
            )
            if success and result is not None:
                total_checks += 1
                shape_violations = _check_output_shape(
                    capability_id, cap_spec.outputs, result
                )
                capability_violations.extend(shape_violations)

                total_checks += 1
                type_violations = _check_output_types(
                    capability_id, cap_spec.outputs, result
                )
        ...
        if capability_violations:
            results["fail"].append(
                {"id": capability_id, "violations": capability_violations}
            )
        else:
