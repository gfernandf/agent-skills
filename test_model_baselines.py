...
check("generate_embedding", len(r["embedding"]) == 8 and r["model"] == "baseline-hash",
      f'{len(r["embedding"])} dims, model={r["model"]}')

# 2. classify_output
r = classify_output({"summary": "A short overview", "points": [1, 2, 3]},
                     ["summary", "code", "error", "list"])
check("classify_output", r["category"] == "summary",
      f'category={r["category"]}, confidence={r["confidence"]}')

# 3. score_output
r = score_output({"text": "The analysis shows growing revenue trends."},
                  "Analyze revenue trends",
                  dimensions=["relevance", "fluency", "completeness"])
check("score_output", "scores" in r and "overall" in r and r["overall"] > 0,
