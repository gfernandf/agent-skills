...
DEFAULT_REPORT = ROOT / "artifacts" / "checkpoint_schema_provenance_report.json"

TEST_NODES: list[str] = [
    "runtime/test_checkpoint.py::TestCheckpointRoundTrip::test_round_trip_dict",
    "runtime/test_checkpoint.py::TestCheckpointRoundTrip::test_minimal_state",
    "runtime/test_workflow_version_compatibility.py::test_checkpoint_restore_rejects_incompatible_state_version",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Verify checkpoint schema and provenance integrity contract for restore, "
            "replay, and fork durability semantics."
        )
    )
    parser.add_argument(
        "--report-file",
...
def main() -> int:
    ...
    print("Running checkpoint schema/provenance verification...")

    tests: list[dict[str, object]] = []
    failures: list[str] = []

    for nodeid in TEST_NODES:
        print(f"[checkpoint-schema] pytest -q {nodeid}")
    ...
    report = {
        ...
        "contract": "checkpoint_schema_provenance_v1",
        "summary": {
            "total_tests": total,
    ...
    print("\nCheckpoint schema/provenance summary")
