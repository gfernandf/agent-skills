...
def test_alternatives_evaluated():
            ...
            "options": [
                {
                    "id": "opt-1",
                    "label": "MVP legaltech",
                    "description": "Minimal legal product",
                },
                {
                    "id": "opt-2",
                    "label": "Full legal suite",
                    "description": "Complete legal platform",
                },
                {
                    "id": "opt-3",
                    "label": "Partnership",
                    "description": "Partner with legal firm",
                },
            ],
    ...
    for i, alt in enumerate(alternatives_eval):
        print(
            f"  [{i}] {alt.get('option')}: score={alt.get('score')}, "
    ...
    assert confidence <= 0.65, (
        f"Domain uncertainty cap not applied: {confidence} > 0.65"
    )
    ...
    if uncertainties:
        ...
        print(
            "  - Uncertainties field not provided; confidence cap used as signal [OK]"
        )
