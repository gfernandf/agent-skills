...
def gate_constraint(payload, gate):
    ...
    """
    if not isinstance(payload, dict):
        return {"decision": "block", "violations": ["payload_must_be_object"], "rationale": "Payload is not a dict."}
    if not isinstance(gate, dict):
        return {"decision": "block", "violations": ["gate_must_be_object"], "rationale": "Gate definition is not a dict."}
    ...
    rationale = "All gate rules satisfied." if not violations else f"{len(violations)} violation(s) detected."
...
def justify_decision(decision, rules, context=None):
    ...
    applicable = [r for r in rules_list if isinstance(r, dict) and r.get("outcome") == decision]
    if not applicable:
        applicable = rules_list[:1]

    rule_ids = [r.get("id", "unknown") for r in applicable]
    justification = (
        f"Decision '{decision}' is justified by {len(applicable)} applicable rule(s): {', '.join(rule_ids)}."
    )
...
def classify_risk(action, categories=None):
    ...
    """
    cats = categories if isinstance(categories, list) and categories else ["low", "medium", "high", "critical"]
    if not isinstance(action, dict):
        return {"risk_level": cats[0], "factors": [], "rationale": "Action is not a dict; defaulting to lowest risk."}
    ...
    if action.get("involves_pii") or action.get("pii"):
        ...
        score += 1
    if action.get("cost") and isinstance(action["cost"], (int, float)) and action["cost"] > 100:
        factors.append("high_cost")
        score += 1

    idx = min(score, len(cats) - 1)
    risk_level = cats[idx]
    rationale = f"Risk classified as '{risk_level}' based on {len(factors)} factor(s)." if factors else f"No risk factors detected; classified as '{risk_level}'."
...
def score_risk(action, dimensions=None):
    ...
    dims = dimensions if isinstance(dimensions, list) and dimensions else [
        "data_exposure", "reversibility", "blast_radius",
    ]
    if not isinstance(action, dict):
        return {"risk_score": 0.0, "dimension_scores": {d: 0.0 for d in dims}, "safe": True}

    dim_scores = {}
    for dim in dims:
        d = str(dim).lower()
        if d in ("data_exposure", "pii"):
            dim_scores[d] = 0.7 if action.get("involves_pii") or action.get("pii") else 0.1
        elif d == "reversibility":
            dim_scores[d] = 0.8 if action.get("irreversible") or action.get("destructive") else 0.1
        elif d == "blast_radius":
            dim_scores[d] = 0.6 if action.get("external") or action.get("public") else 0.2
        else:
            dim_scores[d] = 0.2

    risk_score = round(max(dim_scores.values()) if dim_scores else 0.0, 3)
    return {"risk_score": risk_score, "dimension_scores": {k: round(v, 3) for k, v in dim_scores.items()}, "safe": risk_score < 0.5}
