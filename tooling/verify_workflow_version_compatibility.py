...
DEFAULT_REPORT = ROOT / "artifacts" / "workflow_version_compatibility_report.json"

TEST_NODES: list[str] = [
    "runtime/test_workflow_version_compatibility.py",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Verify workflow version compatibility contract for checkpoint restore, "
            "and replay/fork version propagation semantics."
        )
    )
    parser.add_argument(
        "--report-file",
...
def main() -> int:
    ...
    print("Running workflow version compatibility verification...")

    tests: list[dict[str, object]] = []
    failures: list[str] = []

    for nodeid in TEST_NODES:
        print(f"[workflow-version] pytest -q {nodeid}")
    ...
    report = {
        ...
        "contract": "workflow_version_compatibility_v1",
        "summary": {
            "total_tests": total,
    ...
    print("\nWorkflow version compatibility summary")
